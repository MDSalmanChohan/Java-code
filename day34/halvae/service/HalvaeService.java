...
public class HalvaeService {
    ...
    public void setupHalvaeFromFile() {

        try {

            File myObj = new File("Halvae_Statement.txt");
            Scanner myReader = new Scanner(myObj);
            while (myReader.hasNextLine()) {
                String data = myReader.nextLine();
                String[] lineArray = data.split(",");

                int toper = Integer.parseInt(lineArray[3]);
                Mithai asman = new Mithai();
                asman.name = lineArray[0];
                asman.price = toper;
                asman.state = lineArray[2];
                asman.taste = lineArray[1];
                yusuf.add(asman);
                myReader.close();
